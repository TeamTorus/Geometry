...
CMD_ID = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_VP1304BuilderBatch'
CMD_NAME = 'VP1304 Builder Batch'
CMD_Description = 'Loads control or fit points into sketches. Lofts into propeller. Exports all into folder.'
IS_PROMOTED = True
    ...
    for w in wronger:
        ...
        for s in sketches: 
            if (s.name[0:3] == "Air"):
                ...
                extInput.setSymmetricExtent(adsk.core.ValueInput.createByReal(hubRadius * 8), True) #hub length parameter
        ...
        try:  
            ...
            if (loft is not None):
                ...
                for cps in sketches.itemByName("Airfoil1a").sketchCurves.sketchControlPointSplines:
                    airfoil1.add(cps)
                for sfs in sketches.itemByName("Airfoil1a").sketchCurves.sketchFittedSplines:
                    airfoil1.add(sfs)
                for cps in sketches.itemByName("Airfoil1b").sketchCurves.sketchControlPointSplines:
                    airfoil1.add(cps)
                for sfs in sketches.itemByName("Airfoil1b").sketchCurves.sketchFittedSplines:
                    airfoil1.add(sfs)
                ...
                for cps in sketches.item(density - 2).sketchCurves.sketchControlPointSplines:
                    airfoil2.add(cps)
                for sfs in sketches.item(density - 2).sketchCurves.sketchFittedSplines:
                    airfoil2.add(sfs)
